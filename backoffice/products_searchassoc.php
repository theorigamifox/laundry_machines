<?php
	...
	$theQuery = "select productID,code,name from $tableProducts where (code like \"%$xSearchString%\" or name like \"%$xSearchString%\") order by code,name";
	...
	if ($xOffset-$ordersperpage > -1) {
		...
		$previousButton = "<button id=\"buttonPrev\" class=\"button-expand\" onClick=\"self.location.href='products_searchassoc.php?".userSessionGET().$searchAppend."&xOffset=$pOffset'\">&lt; PREV</button>";
		//$previousButton .= "&nbsp;<button id=\"buttonTop\" class=\"button-expand\" onClick=\"self.location.href='products_searchassoc.php?".userSessionGET().$searchAppend."&xOffset=0'\">[TOP]</button>";
	} else {
		$previousButton = "";
	}
	if ($xOffset+$ordersperpage < $resultCount) {
		$nOffset = $xOffset + $ordersperpage;
		$nextButton = "<button id=\"buttonNext\" class=\"button-expand\" onClick=\"self.location.href='products_searchassoc.php?".userSessionGET().$searchAppend."&xOffset=$nOffset'\">NEXT &gt;</button>";
	} else {
		$nextButton = "";
	}
...
?>
	<tr>
		<td class="table-list-entry1"><a href='javascript:parent.addAssoc(<?php print $ssRecord["productID"]; ?>,"<?php print $ssRecord["code"]; ?>","<?php print $ssRecord["name"]; ?>");'><?php print $ssRecord["code"]; ?></a></td>
		<td class="table-list-entry1"><a href='javascript:parent.addAssoc(<?php print $ssRecord["productID"]; ?>,"<?php print $ssRecord["code"]; ?>","<?php print $ssRecord["name"]; ?>");'><?php print $normalShow; ?></a></td>
	</tr>
<?php
	}
	$dbA->close();
