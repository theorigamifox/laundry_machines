<?php
	...
	$bits = explode(":",$xPaperwork);

	$xPaperworkID = $bits[1];
	$result = $dbA->query("select * from $tablePaperwork where paperworkID = $xPaperworkID");
	if ($dbA->count($result) == 0) {
		echo "INVALID PAPERWORK";
		exit;
	}
	...
	if ($xCmd == "view") {
		grabOrder($xOrderID,$myCount,1,$templateFile);
		echo "<script>parent.pagePrint();</script>";
	}
	if ($xCmd == "print") {
?>
<HTML>
...
<?php
		...
		for ($x=0;$x<count($orderArray); $x++) {
			if ($orderArray[$x] != "") {
				...
				grabOrder($orderID,$myCount,$totalCount, $templateFile);
				if ($onePrinted == true) { echo "</div>"; }
				$onePrinted = true;
				$myCount++;
			}	
		}
		//echo "<script>rc=alert('hi');parent.pagePrint();</script>";
...
<?php
	...
	function grabOrder($xOrderID,$xPage,$totalPages,$xTemplate = "receipt.html") {
		...
		$tpl = new tSys("../templates/",$xTemplate,$requiredVars,0);				
		
		$process = null;
		$process["pagecount"] = $xPage;
		$process["totalpages"] = $totalPages;
		$tpl->addVariable("process",$process);
